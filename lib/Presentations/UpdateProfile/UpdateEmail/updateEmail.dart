...
class _UpdateEmailPageState extends State<ConfirmEmailUpdatePage> {
  ...
  Future<void> validateOtp() async {
    if (_otpController.text.isEmpty || _otpController.text.length != 5) {
      ...
      ApiFunctions().verifyEmailUpdate(context, email,
          _otpController.text.toString(), _emailController.text.toString());
      setState(() {
        checkotp = "";
      });
    }
                      ...
                      GestureDetector(
                        onTap: canResendOtp
                            ? () async {
                                ...
                                String email = prefs.getString('email')!;
                                await ApiFunctions()
                                    .initiateEmailUpdate(context, email);
                                startTimer();
